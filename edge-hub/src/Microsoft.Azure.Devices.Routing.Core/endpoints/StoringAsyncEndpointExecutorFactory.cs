...
namespace Microsoft.Azure.Devices.Routing.Core.Endpoints
{
    ...
    public class StoringAsyncEndpointExecutorFactory : IEndpointExecutorFactory
    {
        ...
        public Task<IEndpointExecutor> CreateAsync(Endpoint endpoint, ImmutableList<uint> priorities) => this.CreateAsync(endpoint, priorities, NullCheckpointer.Instance, this.config);

        public Task<IEndpointExecutor> CreateAsync(Endpoint endpoint, ImmutableList<uint> priorities, ICheckpointer checkpointer) => this.CreateAsync(endpoint, priorities, checkpointer, this.config);

        public async Task<IEndpointExecutor> CreateAsync(Endpoint endpoint, ImmutableList<uint> priorities, ICheckpointer checkpointer, EndpointExecutorConfig endpointExecutorConfig)
        {
