...
namespace Microsoft.Azure.Devices.Routing.Core.Endpoints
{
    ...
    public class AsyncEndpointExecutorFactory : IEndpointExecutorFactory
    {
        ...
        public Task<IEndpointExecutor> CreateAsync(Endpoint endpoint, ImmutableList<uint> _)
        {
        ...
        public Task<IEndpointExecutor> CreateAsync(Endpoint endpoint, ImmutableList<uint> _, ICheckpointer checkpointer)
        {
            IEndpointExecutor executor = new AsyncEndpointExecutor(endpoint, checkpointer, this.config, this.options);
            return Task.FromResult(executor);
        }

        public Task<IEndpointExecutor> CreateAsync(Endpoint endpoint, ImmutableList<uint> _, ICheckpointer checkpointer, EndpointExecutorConfig endpointExecutorConfig)
        {
